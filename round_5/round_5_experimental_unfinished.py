...
class Trader:
    def run(self, state: TradingState):
        ...
        for product in state.order_depths:
            ...
            condition_one = big_dip_checker(products[product].sell_order_history, products[product].buy_order_history, mid_price, 1.08)
            condition_two = small_dip_checker(products[product].sell_order_history, products[product].buy_order_history, 20, mid_price, 1.04)
            condition_three = small_dip_checker(products[product].sell_order_history, products[product].buy_order_history, 10, mid_price, 1.08) and (product in ["PICNIC_BASKET1", "PICNIC_BASKET2"])
            condition_four = small_dip_checker(products[product].sell_order_history, products[product].buy_order_history, 40, mid_price, 1.08) and (product == "DJEMBES")
