...
class Trader:
    def run(self, state: TradingState):
        ...
        for product in state.order_depths:
            ...
            # Continue to the next product because at the end of the day maybe
            # the real profit is the friends we made along the way 
            continue
            ...
            if sell_order_history.get(product) is not None:
                ...
                condition_one = best_ask > (recents_average * 1.04)
                
                # Condition 2 (big-dip checker)
                historical_average = get_average(sell_order_history[product]) 
                condition_two = best_ask > (historical_average * 1.08)
                ...
                if product == "PICNIC_BASKET1" or product == "PICNIC_BASKET2":
                    ...
                    condition_three = best_ask > (recents_average * 1.08)
                ...
                if product == "DJEMBES":
                    ...
                    condition_four = best_ask > (recents_average * 1.08)
